...
import { createTaskId, tasks, type TierSyncLogEvent, type TierSyncSummary } from '@/core/tasks/taskStore';
...
export function startSyncEmbyIdsTask(
...
export function startSyncMovieCountsTask(
  actressIds: Array<string | number>,
  opts?: { onCompleted?: () => void },
) {
  const taskId = createTaskId();
...
export function startTierVideoCountSyncTask(tierId: number, opts?: { onCompleted?: () => void }) {
  const taskId = createTaskId();
  ...
  void (async () => {
    ...
    if (!tier) {
      tasks.set(taskId, { progress: 0, total: 0, status: 'error:tier_not_found', events: [] });
      return;
    }
