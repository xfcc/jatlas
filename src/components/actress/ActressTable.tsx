...
import { Ghost, RefreshCw, ArrowUp, ArrowDown, Unlink } from 'lucide-react';
import { ActressForm } from './ActressForm';
import { BatchActressForm } from './BatchActressForm';
...
import { syncActressVideoCount, batchCreateActresses } from '@/app/actions';
...
export function ActressTable({ actresses: initialActresses, tiers }: ActressTableProps) {
  ...
  const handleBatchCreate = async (data: { tierId: number; names: string }) => {
    const result = await batchCreateActresses(data);

    if (result.success && result.data) {
      toast({
        title: '批量创建成功',
        description: `成功创建 ${result.data.createdCount} 位女优。`,
      });

      if (result.data.skippedCount > 0) {
        toast({
          title: '部分跳过',
          description: `以下 ${result.data.skippedCount} 位已存在: ${result.data.skippedNames.join(', ')}`,
          variant: 'default',
        });
      }
    } else {
      toast({
        title: '批量创建失败',
        description: result.message || '发生未知错误。',
        variant: 'destructive',
      });
    }
    return result;
                ...
                <TableRow key={actress.id} className="bg-zinc-900/20 hover:bg-zinc-900/60 transition-colors cursor-default group">
                  <TableCell className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                        ...
                        {(!actress.emby_id || actress.emby_id.length === 0) && (
                          <Tooltip>
                            <TooltipTrigger>
                    ...
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          className={cn(
                            ...
                            {
                              'opacity-50 cursor-not-allowed': !actress.emby_id || actress.emby_id.length === 0,
                              'pointer-events-none': syncingId !== null
                            }
                          )}
                          title={actress.emby_id && actress.emby_id.length > 0 ? '同步 (Sync from Emby)' : '未绑定 Emby ID'}
                          onClick={() => handleSync(actress)}
                          disabled={!actress.emby_id || actress.emby_id.length === 0 || syncingId !== null}
                        >
         ...
         <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          ...
          <Dialog open={isBatchFormOpen} onOpenChange={setIsBatchFormOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>批量新增女优 (Batch Add)</DialogTitle>
                <DialogDescription>
                  选择一个层级，然后在下方输入框中每行输入一位女优的姓名。
                </DialogDescription>
              </DialogHeader>
              <BatchActressForm 
                tiers={tiers}
                onSave={() => setIsBatchFormOpen(false)}
