import { createTier, getTiers, updateTier, deleteTier } from '@/app/actions';
import { prismaMock } from '../../jest.setup';

jest.mock('next/cache', () => ({
  revalidatePath: jest.fn(),
}));

describe('Tier Actions', () => {
  it('should create a new tier', async () => {
    const tierData = { name: 'Test Tier', video_limit: 10 };
    const expectedTier = { id: 1, ...tierData, created_at: new Date(), updated_at: new Date() };

    prismaMock.tier.create.mockResolvedValue(expectedTier);

    const result = await createTier(tierData);

    expect(prismaMock.tier.create).toHaveBeenCalledWith({ data: tierData });
    expect(require('next/cache').revalidatePath).toHaveBeenCalledWith('/');
    expect(result).toEqual(expectedTier);
  });

  it('should get all tiers', async () => {
    const tiers = [
      { id: 1, name: 'Test Tier 1', video_limit: 10, created_at: new Date(), updated_at: new Date() },
      { id: 2, name: 'Test Tier 2', video_limit: 20, created_at: new Date(), updated_at: new Date() },
    ];
    prismaMock.tier.findMany.mockResolvedValue(tiers);

    const result = await getTiers();

    expect(prismaMock.tier.findMany).toHaveBeenCalled();
    expect(result).toEqual(tiers);
  });

  it('should update a tier', async () => {
    const tierData = { id: 1, name: 'Updated Tier', video_limit: 15 };
    const { id, ...rest } = tierData;
    const expectedTier = { ...tierData, created_at: new Date(), updated_at: new Date() };

    prismaMock.tier.update.mockResolvedValue(expectedTier);

    const result = await updateTier(tierData);

    expect(prismaMock.tier.update).toHaveBeenCalledWith({ where: { id }, data: rest });
    expect(require('next/cache').revalidatePath).toHaveBeenCalledWith('/');
    expect(result).toEqual(expectedTier);
  });

  it('should delete a tier', async () => {
    const tierId = 1;
    const expectedTier = { id: tierId, name: 'deleted', video_limit: 0, created_at: new Date(), updated_at: new Date() };
    prismaMock.tier.delete.mockResolvedValue(expectedTier);

    await deleteTier(tierId);

    expect(prismaMock.tier.delete).toHaveBeenCalledWith({ where: { id: tierId } });
    expect(require('next/cache').revalidatePath).toHaveBeenCalledWith('/');
  });
});
