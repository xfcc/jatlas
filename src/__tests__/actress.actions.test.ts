...
describe('Actress Actions', () => {
  it('should create a new actress', async () => {
    ...
    const expectedActress = { id: 1, ...actressData, emby_id: null, created_at: new Date(), updated_at: new Date(), tier: { id: 1, name: 'Test Tier', video_limit: 10, created_at: new Date(), updated_at: new Date() } };
  ...
  it('should get all actresses', async () => {
    const actresses = [
      { id: 1, name: 'Test Actress 1', video_count: 10, tierId: 1, emby_id: null, created_at: new Date(), updated_at: new Date(), tier: { id: 1, name: 'Test Tier', video_limit: 10, created_at: new Date(), updated_at: new Date() } },
      { id: 2, name: 'Test Actress 2', video_count: 20, tierId: 2, emby_id: null, created_at: new Date(), updated_at: new Date(), tier: { id: 2, name: 'Test Tier 2', video_limit: 20, created_at: new Date(), updated_at: new Date() } },
    ];
    prismaMock.actress.findMany.mockResolvedValue(actresses);

    const result = await getActresses();

    expect(prismaMock.actress.findMany).toHaveBeenCalledWith({ include: { tier: true }, where: {}, orderBy: { id: 'asc' } });
  ...
  it('should update an actress', async () => {
    ...
    const expectedActress = { id: 1, name: 'Test Actress', ...rest, tierId: 1, emby_id: null, created_at: new Date(), updated_at: new Date(), tier: { id: 1, name: 'Test Tier', video_limit: 10, created_at: new Date(), updated_at: new Date() } };
  ...
  it('should delete an actress', async () => {
    const actressId = 1;
    const expectedActress = { id: actressId, name: 'deleted', video_count: 0, tierId: 1, emby_id: null, created_at: new Date(), updated_at: new Date() };
