...
以下界面均在本地控制台或营销页截取（资源位于 `public/`）。

| 文件 | 说明 |
|------|------|
| `index.png` | 营销首页：产品叙事、核心能力与演进路线入口。 |
| `dashboard.png` | **风控大盘**：生态透视、水位预警与清理待办。 |
| `actress.png` | **演员控制台**：列表筛选、动态水位线与单条 Emby 对账。 |
| `bulkaction.png` | **批量能力**：梯队维度的影片数刷新、从 NAS 挂载路径扫描演员文件夹并三分组比对后一键录入等。 |

![营销首页](./public/index.png)

![风控大盘](./public/dashboard.png)

![演员控制台](./public/actress.png)

![批量操作与存储导入](./public/bulkaction.png)
...
在「梯队字典」中从 **NAS AFP 地址或本机已挂载路径**（如 `/Volumes/Share/JAV_output`）扫描一级子目录名，与系统内演员做 **本梯队一致 / 梯队不符 / 尚未建档** 三类比对，支持勾选后批量录入当前梯队。对账仍以 Emby 为准；**仅当 `video_count` 实际变化时才更新演员的 `updated_at`**，便于用「最近变动」观察真实入库波动。

界面预览见上文 **产品截图**（含 `bulkaction.png`）。
...
| **v1.3** | ✅ Shipped | 物理存储与名册对齐 | 梯队页：挂载路径扫描演员目录名；与库内三分组比对并批量导入；Emby 同步仅在库存变化时刷新 `updated_at`。 |
