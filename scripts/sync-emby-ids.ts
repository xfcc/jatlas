...
import { updateActress } from '../src/app/actions';
...
async function main() {
  ...
  const actressesWithoutEmbyId = actresses.filter(
    (actress) => !actress.emby_id || actress.emby_id.length === 0
  );
  ...
  for (const actress of actressesWithoutEmbyId) {
    ...
    try {
      ...
      if (embyIds.length > 0) {
        ...
        await updateActress({ id: actress.id, emby_id: embyIds });
